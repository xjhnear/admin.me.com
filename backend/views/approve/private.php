...
?>
    ...
    <div class="box-header">
    	<?= Html::a('一键通过', ['privatesuccessonekey', 'type' => 'private', 'page' => isset($_GET['page'])?$_GET['page']:1 ], ['class' => 'btn btn-primary',
    			'data' => [
                'confirm' => '确认一键通过本页宣言?',
                'method' => 'post',
            ],]) ?>
        <a class="btn btn-default">待审核</a>
        <?= Html::a('审核通过', ['privatepass'], ['class' => 'btn btn-primary']) ?>
...
function disp_prompt(id)
  ...
  if (name!=null && name!="")
  {
	  window.location.href = "/approve/privatefail?id="+id+"&back=private&reason="+name;
  }
  else if (name=="")
  {
	  window.location.href = "/approve/privatefail?id="+id+"&back=private";
  }
}
</script>
