...
use backend\models\CreditConfigSearch;
...
class CreditconfigController extends Controller
{
    ...
    {
        $searchModel = new CreditConfigSearch();
    ...
    {
        if (($model = CreditConfig::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionExport()
    {
    	...
    	$path = 'CreditConfig_'.date('Ymd').'.csv';
    ...
    public function actionImport()
    {
    	...
    	if ($post_arr) {
    		if (isset($post_arr['importok']) && $post_arr['importok'] == 1) {
    			...
    			$path .= '\..\uploads\credit_config\CreditConfig_'.date('Ymd').'.csv';
    			$model = new CreditConfig();
    			$column = count($model->attributeLabels()) - 2;
    			$csv = new Csv();
    			...
    			foreach ($import_ok_arr as $item) {
    				...
    				$config_model = CreditConfig::find()->where('id='.$item[0])->one();
    				if ($config_model) {
    					$j = 0;
